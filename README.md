<h1 align="center">Hi 👋, I'm Youssef Jaoua</h1>
<h3 align="center">Quality Assurance Engineer | Software Testing Enthusiast | Learning Appium</h3> 

---

### About Me
After obtaining my engineering degree in Computer Science, I began my career as a mobile developer and later transitioned into **Software Testing**. I have solid experience in **manual, automation, and API testing**, and I am passionate about ensuring product quality while improving test efficiency. I am currently expanding my skills by learning **Appium** for mobile test automation.  

---

### 📫 Contact
- Email: **youssef.jaoua@outlook.com**  
- Website: **[youssefjaoua.com](https://youssefjaoua.com)**  
- Location: **Tunisia**  

---

### 🌱 Skills & Tools
- **Testing:** Manual Testing, Automation Testing, API Testing  
- **Frameworks & Tools:** Selenium, Cucumber, Postman, Swagger, Xray, Jira, Git, IntelliJ IDEA, Appium (learning)  
- **Development:** Flutter, Angular, .NET, Node.js, React, Python  
- **Methodology:** Agile  
- **Other Tools:** Figma, Confluence, GitLab  

---

### ⚡ Fun Fact
I enjoy cooking! 🍳  

---

### Connect with Me
<p align="left">
<a href="https://www.linkedin.com/in/jaouayoussef/" target="blank"><img src="https://img.shields.io/badge/LinkedIn-%230077B5.svg?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/></a>
<a href="https://youssefjaoua.com" target="blank"><img src="https://img.shields.io/badge/Website-%23FF5733.svg?style=for-the-badge&logo=ko-fi&logoColor=white" alt="Website"/></a>
<p align="left"> <img src="https://komarev.com/ghpvc/?username=jaouayoussef&label=Profile%20views&color=0e75b6&style=flat" alt="jaouayoussef" /> </p>
</p>
